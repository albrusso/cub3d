...
void	check_move_u(t_player *player, t_data *d)
{
	...
	map_x = ((player->pos.x + new_x) / SIZE);
	map_y = ((player->pos.y + new_y) / SIZE);
	if (d->m->map[map_y][map_x] != '1' && \
	(d->m->map[map_y][(int)d->p->x / SIZE] != '1' && \
		d->m->map[(int)d->p->y / SIZE][map_x] != '1'))
	{
		player->pos.x += new_x;
		player->pos.y += new_y;
	}
...
static void	check_move_r(t_player *player, t_data *d)
{
	...
	new_x = cos(player->dir) * PLAYER_SPEED;
	new_y = sin(player->dir) * PLAYER_SPEED;
	map_x = ((player->pos.x - new_y) / SIZE);
	map_y = ((player->pos.y + new_x) / SIZE);
	if (d->m->map[map_grid_y][map_x] != '1' && \
	(d->m->map[map_grid_y][(int)d->p->x / SIZE] != '1' && \
		d->m->map[(int)d->p->y / SIZE][map_x] != '1'))
	{
		player->pos.x -= new_y;
		player->pos.y += new_x;
